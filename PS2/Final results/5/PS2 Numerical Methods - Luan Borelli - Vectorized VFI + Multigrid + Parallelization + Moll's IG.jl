...
## Vectorizated VFI + Multigrid + Parallelization + Moll's initial guess   ##
...
@time begin 
    for g in 1:length(k_grids)
        ...
        c_matrix = [z_grid[i]*(k_grids[g][j]^α).-k_grids[g][k].+(1-δ)*k_grids[g][j] for j in 1:length(k_grids[g]), i in 1:length(z_grid), k in 1:length(k_grids[g])] # A three-dimensional array of consumptions, containing all possible consumption values, for all possible combinations of k, z and k'.
        ...
        while maximum(abs.(V_prev - V)) > tol && iter < maxiter # Note that we measure the distance using the sup norm.
            ...
            # The iterative process is initiated.
            @threads for j in 1:length(z_grid)  # Given z... 
                @threads for i in 1:length(k_grids[g]) # Given k...
                    ...
                    policy[i,j] = k_grids[g][argmax(value)] # Capital policy function.
                end
