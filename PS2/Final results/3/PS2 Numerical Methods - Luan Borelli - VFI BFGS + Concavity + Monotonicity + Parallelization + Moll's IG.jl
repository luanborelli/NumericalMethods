...
## VFI BFGS + Exploiting both concavity and monotonicity + Parallelization + Moll's initial guess   ##
...
# Now we redefine it to 1 only at the z_grid loop (i.e., outside the k_grid loop), so that "k" counting is 
# being accumulated during the k_grid for-loop. This makes sense: in this way, for each k in 1:length(k_grid) 
# the algorithm will start to search for the optimal k' beginning at the position of the optimal k' associated 
# with the previous capital. This is exactly the idea of exploiting monotonicity! 
