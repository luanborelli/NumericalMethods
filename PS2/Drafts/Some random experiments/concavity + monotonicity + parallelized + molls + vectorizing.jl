...
## VFI BFGS exploiting concavity, parallelizing, Moll's initial guess         ##
...
################################################################################
# VFI BFGS, exploiting concavity and parallelizing, with Moll's initial guess ##
################################################################################

u(i, j, x) = u(z_grid[j]*k_grid[i]^α + (1-δ)*k_grid[i] - k_grid[x]) 
next_u(i, j, x) = u(z_grid[j]*k_grid[i]^α + (1-δ)*k_grid[i] - k_grid[x+1]) 
u_diffs = [next_u(i, j, n) - u(i, j, n) for i in eachindex(k_grid), j in eachindex(z_grid), n in 1:(length(k_grid) - 1)] 
...
@time begin
    ...
    # Additionally, we enforce an iteration limit to avoid the possibility of infinite loops.
        ...
        EVs = [β * Π[j,:]' * V_prev[n+1,:] - β * Π[j,:]' * V_prev[n,:] for j in eachindex(z_grid), n in 1:(length(k_grid) - 1)] 
        @threads for j in 1:length(z_grid) # The iterative process is initiated.
            k = 1
           for i in 1:length(k_grid)
                    diffs = u_diffs[i, j, k:end] + EVs[j, k:end]
                    if any(x -> x < 0, diffs) == true 
                        k += findfirst(x -> x < 0, diffs) - 1
                    else 
                        k = length(k_grid)
                    end 
