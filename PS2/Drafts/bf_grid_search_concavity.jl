...
##############################################
# EXPLOITING BOTH CONCAVITY AND MONOTONICITY #
##############################################

# Notice that in order to exploit monotonicity we only need to change one line of code. 
# Indeed, this one line is the line that redefines k to 1. Now we redefine it to 1 only at 
# the z_grid loop, so that "k" counting is being accumulated during the k_grid for-loop. 
# This makes sense: in this way, for each k in 1:length(k_grid) the algorithm will start
# to search for the optimal k' beginning at the position of the optimal k' associated with the previous capital. 
# This is exactly the idea of exploiting monotonicity! 
...
# É interessante notar que, no caso vetorizado, este truque de explorar a monotinicidade  (ou a concavidade) 
# não faz sentido, uma vez que a vetorização já elimina completamente o terceiro "for" do algoritmo acima. 



# Plots for concavity checking:  

# final_EV = [Π[j,:]' * V[n,:] for n in 1:length(k_grid), j in 1:length(z_grid)] # Matriz de valores esperados, para cada combinação possível de k' e z.
# H = u_matrix[1,1,:] + β*final_EV[:,1] 
# plot(k_grid, H)
