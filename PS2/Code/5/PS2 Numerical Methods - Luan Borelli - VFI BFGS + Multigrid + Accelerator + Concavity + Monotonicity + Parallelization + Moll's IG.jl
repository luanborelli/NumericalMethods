...
## VFI BFGS + Multigrid + Accelerator + Monotonicity + Concavity + Parallelization + Moll's initial guess    ##
...
grid_sizes = [100, 200, 5000] # A vector containing the grid sizes that will be considered in the multigrid method. 
            ...
            for h in 1:n_h # Acceleration starts here. We perform n_h additional iterations *without maximization*, using the previously approximated policy function.
                ...
                @threads for j in 1:length(z_grid) 
                    @threads for i in 1:length(k_grids[g])
                        V[i, j] = u(z_grid[j]*k_grids[g][i]^α + (1-δ)*k_grids[g][i] - policy[i,j]) + β * Π[j,:]' * V_prev[trunc(Int, policy_index[i,j]),:] 
                    end
                end
                iter += 1; 
                print("\n", "Iter: ", iter)
