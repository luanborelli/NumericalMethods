...
a_min = -2;
a_max = +ϕ;
a_grid = range(a_min, a_max, 500); # Note that a small positive perturbation to the lower bound is required so that agents are not able to borrow infinitely.
...
plot(repeat(a_grid,1,9), repeat(z_grid',length(a_grid)), π_∞,
    xlab = "a", 
    ylab = "z", 
    zlab = "π",
    seriestype=:surface,)
...
# @time begin
#   r = find_zero(solve_individuals_problem, (0.0415, 0.0416), Bisection())
# end 
